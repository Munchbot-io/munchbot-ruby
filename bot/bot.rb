...
CUISINE_PREFERENCES = ["Thai", "Chinese", "Italian", "Indian", "Mexican", "Vietnamese"].freeze
DIETARY_PREFERENCES = ["None", "Vegetarian", "Gluten", "Kosher", "Halal", "Keto"].freeze
...
Rubotnik.route :message do
  ...
  bind 'how', 'do', 'you', all: true, to: :start_conversation, reply_with: {
   ...
   bind 'where', 'am', 'I', all: true, to: :lookup_location, reply_with: {
    ...
    bind "Sure", to: :cuisine_preferences
    bind "Thai", to: :dietary_preferences
    bind "None", to: :budget_preferences
    bind "cheap", to: :show_thai
    bind "Mexican", to: :mexican_food_flow
    bind "0.2Miles", to: :show_mexican_options
   ...
   default do
     say "Sorry! I couldn't process that! Try again!"
   end
...
Rubotnik.route :postback do
  ...
  bind 'START' do
    postback.typing_on
    sleep 1
    say "Welcome to Munchbot!"
    postback.typing_on
    sleep 2
    say "Can I ask some questions so I can give choices you find the yummiest?", quick_replies: [["Sure", "YES"], "Later"]
    sleep 2
    postback.typing_off
  end
end
